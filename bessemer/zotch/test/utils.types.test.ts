import { Assert, FilterArrayByKey, FilterArrayByValue, PathParamNames } from '@bessemer/zotch/zotch-type-utils'
...
describe('utils.types', () => {
  describe('FilterArrayByValue', () => {
    ...
    it('should filter typed Array by value in declared order', () => {
      ...
      const test1: Assert<FilterArrayByValue<Input, { a: number }>, [{ a: number; b: string }, { a: number; c: boolean }]> = true
      const test2: Assert<FilterArrayByValue<Input, { c: boolean }>, [{ a: number; c: boolean }, { c: boolean }]> = true
      const test3: Assert<FilterArrayByValue<Input, { d: string }>, [{ d: string }]> = true
      const test4: Assert<FilterArrayByValue<Input, { e: number }>, [{ e: number }]> = true
    })
  })
  ...
  describe('FilterArrayByKey', () => {
    ...
    it('should filter typed Array by key in declared order', () => {
      type Input = [{ a: number; b: string }, { a: number; c: boolean }, { c: boolean }, { d: string }, { e: number }]
      const test1: Assert<FilterArrayByKey<Input, 'a'>, [{ a: number; b: string }, { a: number; c: boolean }]> = true
      const test2: Assert<FilterArrayByKey<Input, 'c'>, [{ a: number; c: boolean }, { c: boolean }]> = true
