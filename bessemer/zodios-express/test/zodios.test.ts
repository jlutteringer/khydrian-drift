...
import z from 'zod'
import { apiBuilder, makeErrors } from '@bessemer/zodios'
import { zodiosContext, zodiosRouter2 } from '@bessemer/zodios-express/zodios'
...
const app = express()
const router = zodiosRouter2(userApi, {
  '/users': {
    get: (req, res) => {
      if (req.queryParameters.active) {
  ...
  '/objects/:id': {
    get: (req, res, next) => {
      res.json(req.queryParameters.test)
    },
  },
})
...
describe('router', () => {
  ...
  it('should infer objects in query params', async () => {
    ...
    const result1 = await req.get('/objects/hello?test=%7B%22test%22%3A%22test%22%7D')
    expect(result1.statusCode).toBe(200)
    expect(result1.body).toEqual({
      test: 'test',
    })
