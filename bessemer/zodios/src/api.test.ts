...
import z from 'zod'
import { apiBuilder, makeApi, makeCrudApi, mergeApis, parametersBuilder, Zodios } from './index'
...
describe('makeCrudApi', () => {
  ...
  it('should update a user', async () => {
    const api = makeCrudApi('user', userSchema)
    const client = new Zodios(`http://localhost:${port}`, api)
    const user = await client.updateUser({ id: 2, name: 'test2' }, { params: { id: 2 } })
  ...
  it('should patch a user', async () => {
    const api = makeCrudApi('user', userSchema)
    const client = new Zodios(`http://localhost:${port}`, api)
    const user = await client.patchUser({ name: 'test2' }, { params: { id: 2 } })
    expect(user).toEqual({ id: 2, name: 'test2' })
  })
