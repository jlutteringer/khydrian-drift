 ...
 */
export function omit<T, K extends keyof T>(obj: T | undefined, keys: K[]): Omit<T, K> {
  const ret = { ...obj } as T
 ...
 */
export function pick<T, K extends keyof T>(obj: T | undefined, keys: K[]): Pick<T, K> {
...
export function replacePathParams(config: ReadonlyDeep<AnyZodiosRequestOptions>) {
...
export function findEndpoint(api: ZodiosEndpointDefinitions, method: string, path: string) {
  return api.find((e) => e.method === method && e.path === path)
}

export function findEndpointByAlias(api: ZodiosEndpointDefinitions, alias: string) {
...
export function findEndpointErrorsByPath(api: ZodiosEndpointDefinitions, method: string, path: string, err: AxiosError) {
  const endpoint = findEndpoint(api, method, path)
  return endpoint && err.config && err.config.url && endpoint.method === err.config.method && pathMatchesUrl(endpoint.path, err.config.url)
    ? findEndpointErrors(endpoint, err)
    : undefined
}

export function findEndpointErrorsByAlias(api: ZodiosEndpointDefinitions, alias: string, err: AxiosError) {
...
export function pathMatchesUrl(path: string, url: string) {
  return new RegExp(`^${path.replace(paramsRegExp, () => '([^/]*)')}$`).test(url)
}
