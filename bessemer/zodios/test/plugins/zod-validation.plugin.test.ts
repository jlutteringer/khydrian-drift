...
describe('zodValidationPlugin', () => {
  ...
  describe('request', () => {
    ...
    it('should throw if endpoint is not found', async () => {
      await expect(plugin.request!(api, notExistingConfig)).rejects.toThrowError('No endpoint found for get /notExisting')
    })

    it('should verify parameters', async () => {
      const transformed = await plugin.request!(api, createSampleConfig('/parse'))
    ...
    it('should transform parameters', async () => {
      const transformed = await plugin.request!(api, createSampleConfig('/transform'))
    ...
    it('should transform empty string parameters', async () => {
      const transformed = await plugin.request!(api, createEmptySampleConfig('/transform'))
    ...
    it('should generate default parameter when generateDefaults is activated', async () => {
      const defaulted = await pluginWithDefaults.request!(api, createUndefinedSampleConfig('/defaults'))
    ...
    it('should not transform parameters when transform is disabled', async () => {
      const notTransformed = await pluginWithoutTransform.request!(api, createSampleConfig('/transform'))
    ...
    it('should transform parameters (async)', async () => {
      const transformed = await plugin.request!(api, createSampleConfig('/transformAsync'))
    ...
    it('should not transform parameters (async) when transform is disabled', async () => {
      const notTransformed = await pluginWithoutTransform.request!(api, createSampleConfig('/transformAsync'))
    ...
    it('should throw on unsuccessful parse', async () => {
      ...
      await expect(plugin.request!(api, badConfig)).rejects.toThrowError("Zodios: Invalid Query parameter 'sampleQueryParam'")
    })
  })
  ...
  describe('response', () => {
    ...
    it('should throw if endpoint is not found', async () => {
      await expect(plugin.response!(api, notExistingConfig, createSampleResponse())).rejects.toThrowError('No endpoint found for get /notExisting')
    })

    it('should verify body', async () => {
      const transformed = await plugin.response!(api, createSampleConfig('/parse'), createSampleResponse())
    ...
    it('should transform body', async () => {
      const transformed = await plugin.response!(api, createSampleConfig('/transform'), createSampleResponse())
    ...
    it('should not transform body when transform is disabled', async () => {
      const notTransformed = await pluginWithoutTransform.response!(api, createSampleConfig('/transform'), createSampleResponse())
    ...
    it('should transform body (async)', async () => {
      const transformed = await plugin.response!(api, createSampleConfig('/transformAsync'), createSampleResponse())
    ...
    it('should not transform body (async) when transform is disabled', async () => {
      const notTransformed = await pluginWithoutTransform.response!(api, createSampleConfig('/transformAsync'), createSampleResponse())
  ...
  const createUndefinedSampleConfig = (url: string): AnyZodiosRequestOptions => ({
    method: 'get',
    url,
  })

  const createSampleResponse = ({ headers } = { headers: { 'content-type': 'application/json' } }) =>
    ({
      data: {
        first: '123',
        second: 111,
