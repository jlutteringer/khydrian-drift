...
describe('ZodiosPlugins', () => {
  ...
  it('should throw if plugin is not registered', () => {
    ...
    expect(() => plugins.eject(id)).toThrowError(`Plugin with key 'test-any' is not registered for endpoint 'any-any'`)
  })

  it('should throw if named plugin is not registered', () => {
    const plugins = new ZodiosPlugins('any', 'any')
    expect(() => plugins.eject('test')).toThrowError(`Plugin with name 'test' not found`)
  })
